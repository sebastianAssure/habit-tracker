...
export const HabitCard = ({ habit, onDelete }: HabitCardProps) => {
    const [checkedDays, setCheckedDays] = useState<boolean[]>(habit.checkedDays);
    ...
    return (
        <div className="group flex flex-col w-100 h-45 items-center py-3">
            <div className={`flex w-full ${habit.color} justify-between px-2 border-2 rounded-md`}>
                ...
                <span className="invisible group-hover:visible cursor-pointer" onClick={()=>onDelete(habit.id)}>X</span>
            </div>
